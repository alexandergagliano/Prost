...
class GalaxyCatalog:
    ...
    def get_sources(self, search_rad, pos, timequery=True):
        ...
        if self.name == 'panstarrs':
            self.mlim = 26
            self.galaxies = build_panstarrs_candidates(pos, search_rad)
        elif self.name == 'decals':
            self.mlim = 26
            self.galaxies = build_decals_candidates(pos, search_rad)
        elif self.name == 'glade':
            self.mlim = 17
            if self.data is not None:
                self.galaxies = build_glade_candidates(pos, search_rad, self.data)
            else:
        ...
        if self.ngals >= 1:
            print(f"Success! {self.ngals} gals retrieved in {elapsed:.2f}s.")


class Transient:
    def __init__(self, name, position, redshift=np.nan, spec_class='', phot_class=''):
        self.name = name
    ...
    def set_likelihood(self, type, func):
        ...
        print(f"Likelihood is set for {type}.")

    def set_prior(self, type, func):
        self.priors[type] = func
        print(f"Prior is set for {type}.")

    def set_likelihood(self, type, func):
        self.likes[type] = func
        print(f"Likelihood is set for {type}.")

    def gen_z_samples(self, n_samples=1000):
        if self.redshift != self.redshift:
            self.redshift_samples = np.maximum(0.001, self.get_prior('redshift').rvs(size=n_samples))
        else:
            self.redshift_samples = np.maximum(0.001, norm.rvs(self.redshift, self.redshift_std, size=n_samples))
        print(f"Successfully generated {n_samples} redshift samples.")
    ...
    def associate(self, galaxy_catalog, verbose=False, n_samples=1000):
        ...
        top_idxs = np.argsort(np.nanmedian(P_gals_norm))[::-1]
        ...
        galaxy_catalog.z_prob = np.nanmedian(P_z_norm)
        galaxy_catalog.offset_prob = np.nanmedian(P_offsets_norm)
        galaxy_catalog.absmag_prob = np.nanmedian(P_absmag_norm)
        galaxy_catalog.total_prob = np.nanmedian(P_gals_norm)
    ...
    def probability_host_outside_cone(self, cutout_rad=60, verbose=False, n_samples=1000):
        ...
        if np.isnan(z_sn):
            # draw galaxies from the same distribution
            z_best_mean = self.gen_z_samples(n_samples) #draw from prior if redshift is missing
            ...
            Prior_z = self.calc_prior_redshift(z_best_samples, reduce=None)
            L_z = self.calc_like_offset(galaxies, reduce=None)
...
def build_glade_candidates(transient_pos, search_rad, GLADE_catalog, n_samples=1000):
    ...
    sn_ra, sn_dec = transient_pos.ra.degree, transient_pos.dec.degree
...
def build_decals_candidates(transient_pos, search_rad=60, n_samples=1000):
    ...
    DLR_samples = calc_DLR(transient_pos, galaxies_pos, size, size_std, a_over_b, a_over_b_std, phi, phi_std)
...
def build_panstarrs_candidates(sn_name, transient_pos, search_rad=60, n_samples=1000):
    search_rad = 200
    sn_name = '2023zkd'
    transient_pos = SkyCoord(237.1981094*u.deg, 9.2000414*u.deg)
    rad_deg = search_rad/3600.
